      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-foreground/40 text-sm">Rastrea tus entregas y actividades</p>
          </div>
        </div>
        ...
        {tabs.map((t) => (
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-all ${
              tab === t.key ? 'bg-foreground/10 text-foreground' : 'text-foreground/40 hover:text-foreground/60 hover:bg-foreground/5'
            }`}
          >
      ...
      <div className="glass-card rounded-2xl overflow-hidden">
        {isLoading ? (
          ...
          <div className="divide-y divide-foreground/6">
            {tareas.map((t) => (
              <div key={t.id_tarea} className="flex items-center gap-4 p-4 hover:bg-foreground/5 transition-all">
                {/* Complete button */}
                <button
                  ...
                  className={`w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 transition-all ${
                    t.completada
                      ...
                      : 'border-foreground/20 hover:border-green-400/50'
                  }`}
                >
                ...
                <div className="flex-1 min-w-0">
                  <p className={`text-sm font-medium ${t.completada ? 'text-foreground/40 line-through' : 'text-foreground'}`}>
                    {t.titulo}
                  </p>
                ...
                <div className="flex gap-1">
                  <button onClick={() => openEdit(t)} className="p-2 rounded-lg text-foreground/30 hover:text-blue-400 hover:bg-blue-400/10 transition-all">
                    <Pencil className="w-3.5 h-3.5" />
                  </button>
                  <button onClick={() => deleteMutation.mutate(t.id_tarea)} className="p-2 rounded-lg text-foreground/30 hover:text-red-400 hover:bg-red-400/10 transition-all">
        ...
        <DialogContent className="glass-card border-foreground/10 text-foreground sm:rounded-2xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              ...
              <Input className="glass-input text-foreground border-0" placeholder="Ej: Entregar ensayo" value={form.titulo} onChange={(e) => setForm({ ...form, titulo: e.target.value })} required />
            </div>
            <div className="space-y-2">
              <Label className="text-foreground/70">Descripción</Label>
              <Input className="glass-input text-foreground border-0" placeholder="Detalles opcionales" value={form.descripcion} onChange={(e) => setForm({ ...form, descripcion: e.target.value })} />
            </div>
            <div className="space-y-2">
              <Label className="text-foreground/70">Fecha de entrega</Label>
              <Input type="date" className="glass-input text-foreground border-0" value={form.fecha_entrega} onChange={(e) => setForm({ ...form, fecha_entrega: e.target.value })} required />
            </div>
            {!editing && (
              <div className="space-y-2">
            ...
            <DialogFooter>
              ...
              <Button type="submit" className="glass-button text-foreground">{editing ? 'Guardar' : 'Crear Tarea'}</Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
